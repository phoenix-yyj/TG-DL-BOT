...
Compose 默认以 Unraid 的 `nobody:users`（UID/GID `99:100`）运行应用。容器启动时会修正 `downloads/` 的属主和权限：目录为 `777`，文件为 `666`，因此新下载文件不会再由 `root:root` 创建。`sessions/` 和 `attached_assets/` 仍保持私有权限，避免会话或配置中的凭据被其他用户读取。若 NAS 使用其他账号，可在 `.env` 中设置 `PUID` 和 `PGID`。
...
文件生命周期：本地合集目录使用 `tmp/` 作为下载和处理中的工作区；自动监听则将源文件保存到对应正式目录旁的 `.sources/<规则目录>/`，处理产物才写入配置的 `output_dir`。只有处理成功的产物会进入正式目录；未命中规则、等待分卷或处理失败的源文件保留在独立源目录（下载中断的临时文件进入其 `failed/` 子目录），避免源压缩包混入正式产物目录。自动监听会对同一分卷组加锁，并在已有完整产物时幂等恢复状态。
...
自动监听处理同样使用 `archive_rules.json`：推荐在规则中使用稳定的 `chat`（公开用户名或配置中的
监听 peer）匹配来源，避免群标题变更导致规则失效；未配置 `chat` 时兼容按 `chat_title` 精确匹配。
随后执行对应的 `steps`。自动监听配置中的 `passwords` 非空时会覆盖规则文件的通用密码表。没有
匹配规则时保留原始压缩包，不再强制重新打包为 `_unlocked.zip`。
